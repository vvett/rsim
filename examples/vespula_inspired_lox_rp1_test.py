...
TARGET_LOX_FLOW = 3.2
TARGET_RP1_FLOW = 1.6
LOX_INTERNAL_VOLUME = 70.0 * LITERS_TO_M3
...
def make_native_tank(
    ...
    *,
    ...
    return rsim.Tank(
        ...
        collapse_factor=1.0,
        blowdown_mode=rsim.BlowdownMode.ENERGY_CONSERVING,
...
def make_aerodynamics() -> tuple[rsim.Aerodynamics, rsim.aero.RocketGeometry]:
    ...
    grid = rsim.aero.AeroGrid(
        ...
        alpha_deg=rsim.aero.GridAxis([-10.0, -5.0, -2.5, 0.0, 2.5, 5.0, 10.0]),
        phi_aero_deg=rsim.aero.GridAxis([0.0, 20.0, 45.0, 90.0]),
        reynolds=rsim.aero.GridAxis([1.0e5, 1.0e6, 1.0e7, 3.0e7, 3.0e10]),
    )
...
# Tank depletion natively flames out the combustor and removes engine thrust.
# The truth event then disables the network after chamber pressure falls below 10 psi.
burnout = rsim.TruthEvent(
    "combustor pressure below 10 psi",
    f"{engine.state_namespace}.chamberPressure less_than {10.0 * PSI_TO_PA}",
)
burnout.add_model_enable_command(propulsion, False)
...
simulator.set_vehicle_state_time_steps(vehicle, {
    rsim.VehicleState.ON_RAIL: TIME_STEP,
    rsim.VehicleState.THRUSTING: TIME_STEP,
    rsim.VehicleState.COASTING: TIME_STEP,
    rsim.VehicleState.DESCENDING: TIME_STEP,
    rsim.VehicleState.DROGUE_DEPLOYED: TIME_STEP,
    rsim.VehicleState.MAIN_DEPLOYED: TIME_STEP,
})
...
assert burnout.fired and not propulsion.enabled
...
assert data[f"{engine.state_namespace}.chamberPressure"][-1] < 10.0 * PSI_TO_PA
...
plt.plot(data['time'], data['Vespula_like_aerodynamics.forceX'], label='Drag')
plt.plot(data['time'], data['Vespula_class_illustrative_engine.thrust'], label='Thrust')
plt.xlabel('Time')
plt.ylabel('Force (N)')
plt.xlim(0, 100)
plt.legend()
...
plt.plot(data['time'], data['native_flight_conditions.dynamicPressure'])
plt.xlabel('Time')
plt.yscale('log')
plt.ylabel('qdyn (Pa)')
plt.xlim(0, 100)
...
plt.plot(data['time'], data['native_flight_conditions.altitude'])
plt.xlabel('Time')
...
plt.plot(data['time'], data['shared_N2_pressure_fed_LOX_RP_1.RP_1_tank.pressure'] * 0.000145038, label='Fuel Tank')
plt.plot(data['time'], data['shared_N2_pressure_fed_LOX_RP_1.LOX_tank.pressure'] * 0.000145038, label='LOX Tank')
plt.plot(data['time'], data['shared_N2_pressure_fed_LOX_RP_1.shared_20_L_N2_COPV.pressure'] * 0.000145038, label='COPV')
plt.plot(data['time'], data['shared_N2_pressure_fed_LOX_RP_1.Vespula_class_illustrative_engine.pressure'] * 0.000145038, label='Pcc')
plt.xlabel('Time')
